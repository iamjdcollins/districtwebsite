...
def disable_missing(existing_employees):
    for employee in existing_employees:
        employee = Employee.objects.get(uuid=employee)
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        employee_nodes = Employee.objects.filter(is_active=True)
        existing_employees = {}
        for node in employee_nodes:
            existing_employees[str(node.pk).lower()] = str(node.pk).lower()
        ...
        for item in conn.entries:
            importUser(item, importuserssvc,departments,all_adult_staff,website_managers)
            existing_employees.pop(str(item.objectGUID).lower(), None)
        disable_missing(existing_employees)
